...
const EditClienteForm = () => {
    ...
    const handleIdentificacionBlur = async () => {
        if (formData.cli_identificacion === originalIdentificacion)
            return; // no cambió
    ...
    const handleCelularBlur = () => {
        if (formData.cli_celular === originalCelular)
            return; // no cambió

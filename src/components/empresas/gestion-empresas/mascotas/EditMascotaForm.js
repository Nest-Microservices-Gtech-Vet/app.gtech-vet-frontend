...
const EditMascotaForm = () => {
    ...
    const filteredClientes = clientes.filter(cli => `${cli.cli_nombre} ${cli.cli_apellido}`.toLowerCase().includes(searchCliente.toLowerCase()));
    useEffect(() => {
        const fetchMascota = async () => {
            try {
                ...
                if (mascota) {
                    ...
                    const clienteActual = clientesRes.data.find((cli) => cli.cli_id === mascotaData.cliente_id);
                    if (clienteActual) {
    ...
    return (_jsxs("div", { className: "w-full max-w-screen-xl mx-auto mb-4", children: [_jsx("div", { className: "flex flex-wrap justify-items-start items-center px-6 py-4", children: _jsxs("div", { className: "flex gap-3 mt-2 lg:mt-0 flex-wrap", children: [_jsx("button", { onClick: () => navigate(`/mis-empresas/${id}/mascotas`), className: "bg-purple-500 hover:bg-purple-600 text-white py-2 px-4 rounded-md transition", children: "\uD83D\uDD19 Volver a Listado Mascotas" }), _jsx("button", { onClick: () => navigate(`/mis-empresas/${id}/mascotas/${mascotaId}/historia-clinica`), className: "bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-md transition", children: "\uD83E\uDE7A Historial Cl\u00EDnico" }), _jsx("button", { onClick: () => navigate(`/mis-empresas/${id}/mascotas/editar-mascota/${mascotaId}/historia-clinica/nueva`), className: "bg-emerald-500 hover:bg-emerald-600 text-white py-2 px-4 rounded-md transition", children: "\u2795 Nueva Consulta" }), _jsx("button", { onClick: () => navigate(`/mis-empresas/${id}/mascotas/${mascotaId}/vacunas`), className: "bg-sky-500 hover:bg-sky-600 text-white py-2 px-4 rounded-md transition", children: "\uD83D\uDC89 Vacunas" })] }) }), _jsxs("form", { onSubmit: updatedMascotaq, className: "w-full max-w-screen-xl mx-auto p-8 rounded-xl shadow bg-white", children: [_jsxs("div", { className: "grid grid-cols-1 lg:grid-cols-3 gap-8", children: [_jsxs("div", { className: "lg:col-span-2 border-r border-gray-200 pr-6", children: [_jsx("h2", { className: "text-2xl font-semibold mb-6 text-gray-800", children: "\uD83D\uDC3E Datos de la Mascota" }), _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-x-6 gap-y-6 justify-center", children: [_jsxs("div", { className: "col-span-full just", children: [typeof formData.mas_foto === "string" && (_jsxs("div", { className: "mb-2", children: [_jsx("label", { className: "block text-sm font-medium text-gray-700", children: "Foto actual" }), _jsx("img", { src: `https://app.amigovet123.com:8443/uploads/perfil/${formData.mas_foto}`, alt: "Foto mascota", className: "w-32 h-32 object-cover rounded border" })] })), formData.mas_foto instanceof File && (_jsxs("div", { className: "mb-2", children: [_jsx("label", { className: "block text-sm font-medium text-gray-700", children: "Nueva foto seleccionada" }), _jsx("img", { src: URL.createObjectURL(formData.mas_foto), alt: "Nueva foto", className: "w-32 h-32 object-cover rounded border" })] })), _jsxs("label", { className: "bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-md cursor-pointer", children: ["Cambiar foto", _jsx("input", { type: "file", accept: "image/*", className: "hidden", onChange: (e) => {
                                        ...
                                        }, onFocus: () => setShowDropdown(true), onBlur: () => setTimeout(() => setShowDropdown(false), 150), placeholder: "Busca un cliente", className: "w-full h-10 px-3 border border-gray-300 rounded-lg focus:ring-sky-600 focus:outline-none" }), showDropdown && searchCliente && (_jsx("ul", { className: "border border-gray-300 rounded bg-white max-h-40 overflow-y-auto mt-1", children: clientes
                                            .filter(cli => `${cli.cli_nombre} ${cli.cli_apellido}`.toLowerCase().includes(searchCliente.toLowerCase()))
                                            .map(cli => (_jsxs("li", { onClick: () => {
                                                setFormData({ ...formData, cliente_id: cli.cli_id });
                                                setSearchCliente(`${cli.cli_nombre} ${cli.cli_apellido}`);
                                                setShowDropdown(false);
                                            }, className: "px-3 py-2 hover:bg-sky-200 cursor-pointer", children: [cli.cli_nombre, " ", cli.cli_apellido] }, cli.cli_id))) })), _jsx("button", { type: "button", onClick: () => navigate(`/mis-empresas/${id}/clientes/crear-cliente?returnTo=/mis-empresas/${id}/mascotas/editar-mascota/${mascotaId}`), className: "mt-4 w-full bg-green-500 text-white py-2 rounded-md hover:bg-green-600 transition", children: "+ Crear Propietario" })] })] }), " ", _jsxs("div", { className: "mt-10 flex justify-center", children: [" ", _jsxs("div", { className: "flex justify-center mt-4", children: [" ", _jsx("button", { className: "mt-4 min-w-2xl bg-green-500 py-2 rounded-md hover:bg-green-600 items-center", type: "submit", children: "Guardar Cambios" }), " "] }), " "] })] })] }));
};
